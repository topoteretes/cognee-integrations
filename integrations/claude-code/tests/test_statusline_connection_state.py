...
# Built from the renderer's own constants so the colour policy is asserted in one
# place: green ● for a healthy connection, red ✕ + reason for a broken one.
_READY = sl._ok_glyph()


def _fail(state):
    """Expected glyph for a marker `state`, mapped through the display vocabulary."""
    return sl._fail_glyph(sl._REASON_LABELS.get(state, state))
...
def test_own_failure_record_renders_its_reason():
    with _Markers(mine=_rec("auth_failed", _MINE)):
        assert sl._health_prefix(_MINE) == _fail("auth_failed")
...
def test_failure_and_its_reason_are_both_red():
    ...
    assert f"✕ ({sl._COGNEE_KEY_REASON})" in plain, plain
    assert out.index("\033[0m") > out.index(sl._COGNEE_KEY_REASON), "reason must be inside the red"


def test_failure_is_detected_despite_the_colour_prefix():
    """_status_prefix must not use startswith('✕') — an escape now precedes it."""
    with _Markers(mine=_rec("unreachable", _MINE)):
        assert sl._status_prefix(_MINE) == _fail("unreachable")
