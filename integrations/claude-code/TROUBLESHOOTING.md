# Troubleshooting Guide - Claude Code Integration

This guide documents the 5 most common operational failures encountered when running the Claude Code Cognee memory plugin, detailing their symptoms, root causes, and step-by-step resolutions.

---

## 1. Cloud Cold-Start Latency
### Symptom
* When starting a Claude session or submitting the first prompt, the plugin hangs for 15–30 seconds, or prints a timeout/network error, but works instantly on subsequent prompts.
...
### Fix
* **Allow Warm-up**: Wait 30 seconds and run a simple command (e.g. `whoami` or a simple question) to wake up the server.
* **Network Health Check**: Ping your `COGNEE_BASE_URL` directly in the browser or terminal before starting Claude Code to verify it is online.

---

## 2. Embedding-Dimension Mismatch
### Symptom
* Memory queries return empty results silently or trigger database exceptions. Logs in `~/.cognee-plugin/claude-code/hook.log` show vector database dimension errors.
...
### Symptom
* Claude Code plugin fails to bootstrap, or throws `SyntaxError` / `ModuleNotFoundError: No module named 'cognee'` in the terminal logs.
...
### Fix
* **Restart the Session**: Clear the current Claude Code instance, make sure your configurations are locked, and restart a clean session:
  ```bash
  # Remove and reinstall the plugin to force a clean handshake
  claude plugin remove cognee-memory
