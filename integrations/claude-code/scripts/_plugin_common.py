...
def apply_cognee_env() -> None:
    ...
    The improve submit timeout (IMPROVE_SUBMIT_TIMEOUT_DEFAULT_SECONDS) was raised
    in the same diagnosis to clear that >=240s retry floor.
    """
...
# identities would silo memory — Claude Code could not recall what Codex
...
def resolve_runtime_mode() -> dict:
    ...
    compatibility. ``_local_api_url_with_source`` always yields a URL — the
...
#: improve-locks: the machine-wide per-session improve lock removed in 1.5.3.
