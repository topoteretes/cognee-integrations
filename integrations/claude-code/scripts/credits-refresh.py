...
Registered async on ``Stop`` AND ``StopFailure``: the user should see what the
finished turn cost as soon as the response completes — not one prompt later —
and an errored turn spent real money too (its recall and every trace write
before the failure). Async means nothing user-visible waits on the fetch.
