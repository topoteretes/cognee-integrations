...
async def _fire_improve_background(dataset: str, session_id: str, user, reason: str) -> None:
    ...
    try:
        if http_api_ready():
            ...
            hook_log(
                "auto_bridge_fired",
                {"reason": reason, "session": session_id, "via": "http_remember", "wrote": wrote,"elapsed_ms":elapsed_ms},
            )
            if wrote:
        ...
        hook_log("auto_bridge_error", {"reason": reason, "error": str(exc)[:200] ,"elapsed_ms" : elapsed_ms})
