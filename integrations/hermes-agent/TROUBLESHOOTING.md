# Troubleshooting Guide - Hermes Agent Integration

This guide documents the 5 most common operational failures encountered when running the Hermes Agent Cognee memory plugin, detailing their symptoms, root causes, and step-by-step resolutions.
...
* **Network Health Check**: Ping your `COGNEE_BASE_URL` directly in the browser or terminal before starting Hermes Agent to verify it is online.
...
### Symptom
* Hermes Agent plugin fails to bootstrap, or throws `SyntaxError` / `ModuleNotFoundError: No module named 'cognee'` in the terminal logs.
...
### Fix
* **Restart the Session**: Clear the current Hermes Agent instance, make sure your configurations are locked, and restart a clean session.
