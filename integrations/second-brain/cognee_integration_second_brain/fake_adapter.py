"""In-memory chat-memory adapter for deterministic, no-key tests and demos.

Stores items in a plain ``{dataset -> [items]}`` dict and does substring
recall. It carries no cognee dependency and needs no API keys or server, so the
bot's identity, routing, and forget logic can be proven offline in under a
second.
