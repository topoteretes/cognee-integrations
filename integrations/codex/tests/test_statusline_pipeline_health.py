"""Tests for `_pipeline_health_glyph` (cognee_statusline_render.py, Codex) — the
passive, app-closed-safe surface for the pipeline-health sweep's findings,
ported from the claude-code renderer (its twin test is
integrations/claude-code/tests/test_statusline_pipeline_health.py). The glyph is
plain text in Codex (the status is injected into model context, not a terminal
bar) and reads the machine-wide, integration-neutral
``~/.cognee-plugin/pipeline-health.json`` the sweep writes. See
...
Run: python integrations/codex/tests/test_statusline_pipeline_health.py
(or via pytest).
"""
...
# ── wiring: the glyph leads the injected status string ───────────────────


def test_glyph_prefixes_render_status_for_host():
    """The header path (session-context-lookup → render_status_for_host) is the
    primary Codex surface — the glyph must lead the string there, ahead of the
    connection glyph and the `cognee:` core."""
    tmp = _tmp_path()
    _write(
        tmp,
        {
    ...
    try:
        sl._PIPELINE_HEALTH_PATH = tmp
        rendered = sl.render_status_for_host("session-abc")
        assert rendered.startswith("⚠ server-down ")
        assert "cognee: " in rendered
    finally:
        sl._PIPELINE_HEALTH_PATH = orig
        shutil.rmtree(tmp.parent, ignore_errors=True)
