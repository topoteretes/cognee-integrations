...
Run: python integrations/codex/plugins/cognee/tests/test_agent_register_e2e.py (or via pytest).
"""
...
def test_started_session_registers_and_is_listable():
    with _AgentsServer():
        session_key = pc._sanitize_session_key("codex_e2e-listable-session")
...
def test_unregistered_session_is_not_listed():
    ...
    with _AgentsServer():
        resolved = pc.load_resolved(pc._sanitize_session_key("codex_e2e-unregistered-session"))
