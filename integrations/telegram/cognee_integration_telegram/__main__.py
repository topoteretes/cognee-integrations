"""Run the bot with long polling: ``python -m cognee_integration_telegram``.

No public URL or webhook needed — long polling works from a laptop. Requires
``TELEGRAM_BOT_TOKEN`` and a running cognee server (``COGNEE_BASE_URL``, default
``http://localhost:8000``; ``COGNEE_API_KEY`` if that server has auth enabled).
"""
...
def main() -> None:
    ...
    logging.getLogger("cognee_integration_telegram").info(
        "Bot starting (long polling). Press Ctrl-C to stop."
    )
