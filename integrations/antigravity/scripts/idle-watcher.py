#!/usr/bin/env python3
"""Idle watcher daemon - persists quiet Antigravity sessions into Cognee.

Launched detached from ``session-start.py``. Polls
``~/.cognee-plugin/antigravity/activity.ts`` every ``POLL_SECONDS``. When the last
...
Stops cleanly on:
  * ``~/.cognee-plugin/antigravity/watcher.stop`` sentinel file.
