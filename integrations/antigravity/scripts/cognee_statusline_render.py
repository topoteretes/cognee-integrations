#!/usr/bin/env python3
"""Render the Cognee status line (Antigravity).
...
# in the shared root, NOT under an integration state directory — so all integrations read it.
_PIPELINE_HEALTH_PATH = _SHARED_ROOT / "pipeline-health.json"
_UPDATE_CHECK_PATH = _SHARED_ROOT / "antigravity" / "update-check.json"
_LLM_STATE_PATH = _SHARED_ROOT / "antigravity" / "llm-state.json"
...
_LLM_STATE_DIR = _SHARED_ROOT / "antigravity" / "llm-state"
_CONN_STATE_DIR = _SHARED_ROOT / "antigravity" / "conn-state"
...
_CREDITS_PATH = _SHARED_ROOT / "antigravity" / "credits.json"
...
def _active_dataset() -> str:
    # 1. env var (inherited from the shell that launched Antigravity)
...
def _update_segment() -> str:
    ...
    Antigravity surfaces status inside the model's context (not a terminal bar), so this
...
def _pipeline_health_glyph() -> str:
    ...
    clean). Antigravity copy of the shared renderer glyph (kept in sync by hand —
    ...
    Antigravity session running the plugin. See docs/KB/pipeline-monitor-notify-policy.md
...
def _llm_prefix(session_id: str = "") -> str:
    ...
    since Antigravity injects the status into model context, not a terminal bar.
...
def _credits_segment() -> str:
    ...
    tenant's entry by that binding. Plain text (the Antigravity line carries no ANSI
