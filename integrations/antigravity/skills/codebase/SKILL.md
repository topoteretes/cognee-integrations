---
name: codebase
description: Use when indexing a code repository into Cognee's code graph or querying it (callers, impact analysis, paths, endpoints) from Antigravity.
---

# Cognee Code Graph

Use this skill when the user asks Antigravity to build a Cognee memory of a
...
```bash
"${COGNEE_ANTIGRAVITY_PLUGIN_ROOT:-$HOME/.gemini/config/plugins/cognee}"/scripts/cognee-index-repo.sh <repo-path-or-git-url> [--dataset <name>] [--index-vectors] [--wait <seconds>]
```
...
```bash
"${COGNEE_ANTIGRAVITY_PLUGIN_ROOT:-$HOME/.gemini/config/plugins/cognee}"/scripts/cognee-search.sh "<seed>" 10 --code [--code-query '<json>']
```
