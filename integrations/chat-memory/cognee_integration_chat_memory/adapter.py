...
from .backend import CogneeHttpMemoryBackend, MemoryBackend, deterministic_item_id
...
logger = logging.getLogger("chat_memory.adapter")
...
class ChatMemoryAdapter:
    ...
    Args:
        ...
        backend: The memory backend. Defaults to :class:`CogneeHttpMemoryBackend`
            (talks to a running cognee server over HTTP; reads ``COGNEE_BASE_URL``
            / ``COGNEE_API_KEY`` from the environment). Inject
            :class:`InMemoryMemoryBackend` in tests, or
            :class:`CogneeMemoryBackend` (the optional ``[sdk]`` extra) for
            in-process cognee.
    ...
    def __init__(
        ...
        *,
        ...
        self._scope = scope
        self.backend: MemoryBackend = backend or CogneeHttpMemoryBackend()
