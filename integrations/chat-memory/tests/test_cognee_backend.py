...
The in-process SDK backend is optional (the ``[sdk]`` extra), so this whole
module is skipped when cognee is not installed — CI runs keyless and does not
install cognee. When it *is* present, these monkeypatch ``cognee.remember`` /
``recall`` / ``forget`` plus the dataset/user lookups to assert the exact calls
the backend makes without an LLM or a database:
...
cognee = pytest.importorskip("cognee", reason="install the [sdk] extra to test CogneeMemoryBackend")

from cognee_integration_chat_memory import CogneeMemoryBackend  # noqa: E402
