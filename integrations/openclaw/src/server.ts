...
const EXIT_WATCHER_CONTENT = [
  ...
  "# Same ceiling as the Python plugins' logs (COGNEE_LOG_MAX_BYTES, default 20 MiB):",
  "# rotate to .1 when over it, so a watcher that logs for months stays readable.",
  "try: LOG_MAX = max(0, int(float(os.environ.get('COGNEE_LOG_MAX_BYTES', '') or 20 * 1024 * 1024)))",
