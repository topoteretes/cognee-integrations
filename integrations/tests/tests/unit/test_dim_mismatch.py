...
Migrated from {claude-code,codex}/tests/test_dim_mismatch.py.
"""
...
def test_mismatch_names_both_dims_and_model(pc):
    msg = _hint(pc, _FakeLanceEngine(query_size=3072, stored_vector=[0.0] * 1536))
    assert msg is not None
    assert "1536" in msg and "3072" in msg
    assert "text-embedding-3-large" in msg and "openai" in msg


def test_matching_dims_returns_none(pc):
    assert _hint(pc, _FakeLanceEngine(query_size=1536, stored_vector=[0.0] * 1536)) is None


def test_enumerates_nonstandard_collection(pc):
...
def test_no_collections_returns_none(pc):
    engine = _FakeLanceEngine(query_size=3072, stored_vector=[0.0] * 1536, present=())
    assert _hint(pc, engine) is None


def test_empty_collection_returns_none(pc):
    # Collection present but holds no rows -> normal fresh/empty state, no hint.
    assert _hint(pc, _FakeLanceEngine(query_size=3072, stored_vector=None)) is None


def test_unreadable_store_returns_none(pc):
    ...
    assert _hint(pc, _FakeErrorEngine(query_size=3072)) is None


def test_broken_engine_returns_none(pc):
    class _Broken:
        @property
        def embedding_engine(self):
            raise RuntimeError("boom")

    assert _hint(pc, _Broken()) is None


def test_dim_memo_roundtrip(pc, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(pc, "_DIM_MEMO_FILE", tmp_path / "dim_check.json")
