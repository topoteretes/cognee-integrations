"""Harness self-tests: the mock Cognee server and identity fake over real HTTP.

These exercise the infrastructure itself (not the integrations) so the
integration tests built on it can trust every route and knob.
"""
...
def test_forced_response_short_circuits_a_route(mock_server):
    mock_server.force_response("POST", "/api/v1/recall", 503, {"detail": "down"})
    try:
        _post_json(f"{mock_server.url}/api/v1/recall", {"query": "q"})
        raise AssertionError("expected HTTP 503")
    except urllib.error.HTTPError as exc:
        assert exc.code == 503
    mock_server.assert_called("POST", "/api/v1/recall")

    mock_server.clear_forced("POST", "/api/v1/recall")
    status, body = _post_json(f"{mock_server.url}/api/v1/recall", {"query": "q"})
    assert (status, body) == (200, [])
