...
final class OnboardingModel: ObservableObject {
    ...
    func loadExisting() {
        ...
        userName = values["SPOTLIGHT_USER"] ?? userName
        teamName = values["SPOTLIGHT_TEAM"] ?? ""
    }
    ...
    private func writeConfig() throws {
        var lines = [
            "# Written by Cognee Spotlight (menu bar → Setup…). Sourced by run_backend.sh;",
            "# values here win over the repo .env.",
            "COGNEE_MODE=\(mode.rawValue)",
            "SPOTLIGHT_PORT=\(Profiles.port(profile))",
        ]
        ...
        add("SPOTLIGHT_USER", userName)
        add("SPOTLIGHT_TEAM", teamName)
...
enum BackendLauncher {
    ...
    static func restart(profile: String = "default") async -> Bool {
        ...
        let command =
            ...
            + "SPOTLIGHT_PROFILE='\(profile)' nohup '\(script)' "
            + "> /tmp/spotlight-backend-\(profile).log 2>&1 &"
        let process = Process()
