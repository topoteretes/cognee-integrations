...
Search merges two sources the way Spotlight does: instant filename matches
...
def create_app(
    ...
    app = FastAPI(title="Cognee Spotlight backend")
    ...
    async def share(request: ShareRequest) -> dict:
        if handover is None:
            return {
                "ok": False,
                "error": "handover not configured (set SPOTLIGHT_USER and COGNEE_HUB_URL)",
            }
    ...
    cache_ttl = float(os.getenv("SPOTLIGHT_SEARCH_CACHE_TTL", "600"))
...
def _layer_label(dataset: str) -> str:
