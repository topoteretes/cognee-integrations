...
# Per-agent-session buffer dirs. Each agent session (one Grok/Claude/Codex terminal)
...
def plugin_root_env() -> str:
    """The plugin's install root as the host exports it to plugin hooks.

    Grok Build sets ``GROK_PLUGIN_ROOT`` for plugin hooks; the Claude Code name is
    accepted too because Grok loads Claude Code plugins unchanged and the same
    scripts may run under either contract. '' when neither is set.
    """
    for var in ("GROK_PLUGIN_ROOT", "CLAUDE_PLUGIN_ROOT"):
...
# Grok's native hook payload uses camelCase keys (``hookEventName``, ``sessionId``,
# ``cwd``, ``workspaceRoot``, ``toolName``, ``toolInput``); the Claude-compatible
# path (and these scripts) use snake_case. Both spellings are kept on the dict so
# every reader below works unchanged whichever contract the host speaks.
_PAYLOAD_KEY_ALIASES = {
    ...
    # Grok's PostToolUse carries the tool output as ``toolResult`` (its docs call
    # this out as the one field that differs from Claude's ``tool_response``).
...
def normalize_hook_payload(payload):
    """Return ``payload`` with snake_case aliases for Grok's camelCase hook keys.

    Non-dict input is returned as ``{}``. Existing snake_case keys always win.
    ``cwd`` falls back to ``GROK_WORKSPACE_ROOT`` (exported to every Grok hook)
    when the payload carries neither ``cwd`` nor ``workspaceRoot``.
    """
    if not isinstance(payload, dict):
        return {}
    out = dict(payload)
    for camel, snake in _PAYLOAD_KEY_ALIASES.items():
        if camel in out and snake not in out:
            out[snake] = out[camel]
    if not out.get("cwd"):
        fallback = out.get("workspace_root") or os.environ.get("GROK_WORKSPACE_ROOT", "")
        if fallback:
            out["cwd"] = fallback
    return out
...
def _generate_session_id(cwd: str = "", host_key: str = "") -> str:
    ...
    The host (Grok) session id maps 1:1 to the conversation, so embedding it
    ...
    """
    agent = (
        _sanitize_session_key(os.environ.get("COGNEE_SESSION_PREFIX", "") or "grok") or "grok"
    )
    host = _sanitize_session_key(host_key)
    if host:
        return f"{agent}_{host}"
    cwd = cwd or os.environ.get("GROK_CWD") or os.getcwd()
...
def resolve_host_key_outside_hook(cwd: str = "") -> tuple[str, str]:
    ...
    nothing matched:
      ...
      2. The host's own session-id export (``GROK_SESSION_ID``; the Claude Code /
         Codex names are accepted too since Grok Build mirrors that hook contract).
    ...
    for var in ("GROK_SESSION_ID", "CLAUDE_CODE_SESSION_ID", "CODEX_SESSION_ID", "CODEX_THREAD_ID"):
...
def _candidate_host_pids() -> set[int]:
    ...
    pids: set[int] = set()
    for var in ("GROK_PID", "CLAUDE_PID", "CODEX_PID"):
        try:
...
def resolve_session_key_from_payload(payload: dict) -> tuple[str, str]:
    """Resolve session key from a hook payload using known host (Claude-style) variants."""
    if not isinstance(payload, dict):
...
def _resolve_agent_name() -> str:
    ...
    try:
        ...
        pass
    return _normalize("grok-bot-agent")
...
def write_connection_state(
    ...
    Global (not namespaced) because Grok, Claude and Codex share one server on the
...
# The marker is a MAP keyed by tenant id: several concurrent Grok sessions
...
# Grok Build's plugin registry (verified against grok 1.0.13): one file mapping
# installed repos to their source kind ({"type": "Local", "source_path": ...} or a
# git source) and the plugins they provide with versions.
_GROK_PLUGIN_REGISTRY = Path.home() / ".grok" / "installed-plugins" / "registry.json"
# The same registry doubles as the install record: rewritten the moment
# `grok plugin update` installs a new version, so it reflects an update in the
# session it happened in. Read best-effort: a missing file means "no registry".
_INSTALLED_PLUGINS_FILE = _GROK_PLUGIN_REGISTRY
...
def _installed_plugin_version() -> str:
    candidates = []
    root = plugin_root_env()
    if root:
        candidates.append(Path(root) / ".grok-plugin" / "plugin.json")
    candidates.append(Path(__file__).resolve().parent.parent / ".grok-plugin" / "plugin.json")
...
def _recorded_install_version() -> str:
    """Newest cognee-memory version recorded in Grok Build's install registry.
    ...
    try:
        ...
        return ""
    return _newest_registered_version(data, _UPDATE_PLUGIN_ENTRY)


def _registry_entries(data, plugin: str) -> list:
    """(version, source_kind) pairs for ``plugin`` from either registry shape.

    Grok Build: ``{"repos": {<id>: {"kind": {"type": "Local"|..., ...},
    "plugins": {<name>: {"version": ...}}}}}``. Claude Code (which Grok can also
    load plugins from): ``{"plugins": {"<name>@<marketplace>": [{"version": ...}]}}``
    — kept so a registry in that shape still yields a version.
    """
    out: list = []
    if not isinstance(data, dict):
        return out
    ...
    if isinstance(repos, dict):
        for repo in repos.values():
            if not isinstance(repo, dict):
                continue
            entry = (repo.get("plugins") or {}).get(plugin) if isinstance(repo.get("plugins"), dict) else None
            if isinstance(entry, dict):
                kind = repo.get("kind")
                kind_type = str(kind.get("type") or "") if isinstance(kind, dict) else str(kind or "")
                out.append((str(entry.get("version") or ""), kind_type))
        ...
        for key, installs in plugins.items():
            ...
            for install in installs if isinstance(installs, list) else []:
                if isinstance(install, dict):
                    out.append((str(install.get("version") or ""), ""))
    return out


def _newest_registered_version(data, plugin: str) -> str:
    best, best_str = (), ""
    for version, _kind in _registry_entries(data, plugin):
...
def _update_source() -> Optional[tuple]:
    ...
    Reads Grok's plugin registry: when the repo providing cognee-memory was
    installed from a local path (``kind.type == "Local"``, i.e. a dev checkout)
    returns None so local checkouts never nag themselves. Any git-sourced install,
    or a missing/odd registry, falls back to the default repo (the published case).
    """
    try:
        data = json.loads(_GROK_PLUGIN_REGISTRY.read_text(encoding="utf-8"))
        kinds = {kind for _v, kind in _registry_entries(data, _UPDATE_PLUGIN_ENTRY)}
        if kinds and all(kind.lower() == "local" for kind in kinds):
            return None  # local-path install: skip, don't self-nag
    except FileNotFoundError:
        pass
    except Exception as exc:
...
def _fetch_published_version(repo: str, ref: str, etag: str) -> tuple:
    ...
    """
    url = f"https://raw.githubusercontent.com/{repo}/{ref}/.grok-plugin/marketplace.json"
...
def read_update_status() -> dict:
    ...
    - Grok Build's install registry (``installed_plugins.json``) already
      records a version at or past ``latest_version`` — `grok plugin update`
