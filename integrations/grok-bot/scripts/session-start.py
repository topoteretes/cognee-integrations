...
Runs on the SessionStart hook. Responsibilities:
  ...
  5. Register the current Grok session as an active agent connection
"""
...
from _plugin_common import plugin_root_env
...
_STATE_DIR = Path.home() / ".cognee-plugin" / "grok-bot"
...
async def _login_default_user_for_owner_api_key(service_url: str, config: dict) -> str:
    ...
    status, body = _cloud_http_request(
        f"{base}/api/v1/auth/api-keys",
        method="POST",
        json_body={"name": "grok-owner-bootstrap"},
        cookies={"auth_token": jwt},
        timeout=30.0,
    )
    if status != 200:
...
def _resolve_agent_name(config: dict, cwd: str) -> str:
    ...
    return _normalize(f"grok-{Path(cwd).name}")
...
def _find_grok_parent_pid() -> int:
    """Find the nearest live Grok ancestor, skipping hook shells."""
    fallback = os.getppid()
    if sys.platform == "win32":
        return find_host_ancestor_windows(fallback, "grok")
    try:
        ...
        hook_log("find_grok_parent_failed", {"error": str(exc)[:200]})
        return fallback
    ...
    # Match "grok" as an executable basename anywhere in the command line,
    # tolerant of spaces in the executable path (a naive split()[0] mis-tokenizes
    # paths like "/…/Application Support/…/grok").
    host_re = re.compile(r"(?:^|/)grok(?:-[\w.]+)?(?:\s|$)")
    pid = fallback
    seen: set[int] = set()
...
def _spawn_exit_watcher(
    ...
    *,
    ...
    """Launch a detached watcher that syncs only after Grok exits."""
    ...
    parent_pid = _find_grok_parent_pid()
...
def _bootstrap_singleflight():
    ...
    The marker is global because Grok, Claude and Codex share one local server, so
...
def _scripts_dir() -> str:
    """Absolute path of this plugin's scripts directory, for in-context guidance."""
    root = plugin_root_env()
    return str(Path(root) / "scripts") if root else str(Path(__file__).resolve().parent)
...
def _session_start_guidance(mode: str, dataset: str, session_id: str, ready: bool) -> dict:
    if ready:
        message = (
            ...
            "Use the cognee-remember skill to store permanent memory, "
            "and cognee-search to query by node set.\n"
            "Plugin scripts directory (use it where a skill says ${GROK_PLUGIN_ROOT}): "
            f"{_scripts_dir()}"
        )
    else:
        message = (
...
def _log_statusline_mode() -> None:
    """Grok Build has no documented ``statusLine`` setting to wire.

    Claude Code hosts get a terminal status bar written into their settings; Grok
    Build keeps its configuration in ``~/.grok/config.toml`` and documents no
    equivalent, so nothing is written. The per-prompt recall header (the
    ``Cognee memory: ...`` line the UserPromptSubmit hook injects) carries the
    same connection/dataset information in-context, and
    ``scripts/cognee-statusline.sh`` renders the full bar on demand.
    """
    hook_log("statusline_setup_skipped", {"reason": "unsupported_host"})
...
_MEMORY_PREFERENCE_STEER = (
    ...
    "preference to the host's built-in memory files, and do not duplicate durable "
    "knowledge into them. Cognee is the source of truth for memory."
)


def _apply_memory_preference(output: dict) -> dict:
    """Steer the model to treat Cognee as the preferred memory over native auto-memory.

    A host's built-in auto memory can't be reliably disabled by a plugin, so
    instead we assert Cognee as the authoritative memory via a SessionStart
...
def _apply_update_nudge(output: dict) -> dict:
    ...
    message = (
        ...
        "`grok plugin update cognee-memory` (or `grok plugin marketplace update cognee`)."
    )
    result = dict(output or {})
    hso = dict(result.get("hookSpecificOutput") or {})
    hso.setdefault("hookEventName", "SessionStart")
...
async def _start(payload: dict | None = None) -> dict:
    _log_statusline_mode()
    config = load_config()
    payload = payload or {}
    cwd = str(payload.get("cwd") or os.environ.get("GROK_CWD") or os.getcwd())
    ...
    # The host (Grok) session id is a local correlation key only: it keeps every
    ...
    session_id, conn_uuid = ensure_launch_record(
        ...
        host_pid=_find_grok_parent_pid(),
    )
    os.environ["COGNEE_SESSION_ID"] = session_id
    ...
    # Reset the idle clock for this Grok process before the watcher
