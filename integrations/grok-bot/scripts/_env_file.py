...
Grok, Claude Code and Codex plugins) are injected into ``os.environ`` at process
...
# plugin var is the ONE line that differs between the Grok, Claude Code and
# Codex copies of this module.
_PLUGIN_BACKEND_VAR = "COGNEE_GROK_BACKEND"
_SHARED_BACKEND_VAR = "COGNEE_BACKEND"
_LOCAL_BACKEND_VALUES = ("local", "native", "sdk")
_CLOUD_BACKEND_VALUES = ("cloud", "http", "api", "server")

_TEMPLATE = """\
# Cognee plugin configuration — shared by the Grok, Claude Code and Codex plugins.
...
# (COGNEE_GROK_BACKEND / COGNEE_CLAUDE_BACKEND / COGNEE_CODEX_BACKEND target one plugin only.)
