...
parallel context-lookup hook. Unlike the Grok/Claude integrations, Codex keeps
...
async def _store(prompt: str, payload: dict):
    ...
    if runtime["mode"] == "local_sdk" and server_ready_hint(runtime.get("base_url", "")):
        # Keep Cognee initialization parity with the Claude plugin so fresh local
...
def main():
    ...
    try:
        payload = normalize_hook_payload(json.loads(payload_raw))
