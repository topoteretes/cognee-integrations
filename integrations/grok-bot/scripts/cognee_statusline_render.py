...
Invoked via ``cognee-statusline.sh`` (a Claude-Code-style ``statusLine`` command), which
...
_UPDATE_CHECK_PATH = _SHARED_ROOT / "grok-bot" / "update-check.json"
# Grok Build's own install registry: rewritten the moment `grok plugin update`
...
_INSTALLED_PLUGINS_PATH = Path.home() / ".grok" / "installed-plugins" / "registry.json"
_LLM_STATE_PATH = _SHARED_ROOT / "grok-bot" / "llm-state.json"
_RECALL_PATH = _SHARED_ROOT / "grok-bot" / "last_recall.json"
_RECALL_DIR = _SHARED_ROOT / "grok-bot" / "recall"
_CREDITS_PATH = _SHARED_ROOT / "grok-bot" / "credits.json"
...
_LLM_STATE_DIR = _SHARED_ROOT / "grok-bot" / "llm-state"
_CONN_STATE_DIR = _SHARED_ROOT / "grok-bot" / "conn-state"
...
# Self-eviction: when the plugin is explicitly disabled in a settings scope but
# its files still linger on disk, this renderer would otherwise keep drawing a
# status line for a plugin that is no longer active. Grok Build keeps its own
# registry under ~/.grok and documents no ``enabledPlugins`` map, so the check is
# conservative: evict only when some scope lists the plugin and none lists it as
# enabled. No mention anywhere means "enabled" — never render nothing by default.
_PLUGIN_ID = "cognee-memory@cognee"
_USER_SETTINGS = Path.home() / ".grok" / "settings.json"
# A statusLine we consider "ours" to evict — never touch a user's own line.
_OWNED_STATUSLINE_MARKER = "cognee-statusline"


_SESSIONS_DIR = _SHARED_ROOT / "grok-bot" / "sessions"
...
def _active_dataset(session_id: str = "") -> str:
    ...
    if recorded:
        return recorded
    # 2. env var (inherited from the shell that launched Grok)
...
def _update_segment() -> str:
    ...
    # happened. Grok Build (like Claude Code) rewrites installed_plugins.json immediately,
...
def _running_plugin_version() -> str:
    ...
    status line is not a hook, so ``GROK_PLUGIN_ROOT`` is usually unset; the
    install path is version-pinned (``.../cognee-memory/<version>/scripts/``), so
    this file's own location identifies the running version.
    """
    candidates = []
    root = (os.environ.get("GROK_PLUGIN_ROOT") or os.environ.get("CLAUDE_PLUGIN_ROOT") or "").strip()
    if root:
...
def _recorded_install_version() -> str:
    ...
    ``~/.grok/installed-plugins/registry.json`` maps installed repos to the plugins
    they provide (``repos.<id>.plugins.<name>.version``); it is rewritten the
    moment an update lands, regardless of which (possibly older) plugin copy this
    renderer belongs to. A Claude-Code-shaped registry (``plugins.<name>@<mkt>``,
    a list of installs) is read too. '' when the file is missing, unreadable, or
    has no parseable cognee-memory entry. Mirrors _plugin_common (not imported).
    """
    try:
    ...
    if isinstance(repos, dict):
        for repo in repos.values():
            plugins = repo.get("plugins") if isinstance(repo, dict) else None
            entry = plugins.get("cognee-memory") if isinstance(plugins, dict) else None
        ...
        for key, installs in plugins.items():
            ...
            for install in installs if isinstance(installs, list) else []:
                if isinstance(install, dict):
                    versions.append(str(install.get("version") or ""))
    best, best_str = (), ""
...
def _plugin_enabled(cwd: str) -> bool:
    ...
    if cwd:
        try:
            proj = Path(cwd)
            paths += [proj / ".grok" / "settings.json", proj / ".grok" / "settings.local.json"]
        except Exception:
            pass
    states = [_enabled_in(p) for p in paths]
    if all(state is None for state in states):
        return True  # no scope mentions the plugin: nothing to evict on
...
def _recall_segment(session_id: str) -> str:
    ...
    the model. It is omitted when zero.
...
def main() -> None:
    ...
    # the renderer exits non-zero, and the host drops the whole status line
    ...
    ctx = {}
    try:
        ctx = json.load(sys.stdin)  # consume stdin as the host requires
    except Exception:
        ctx = {}
