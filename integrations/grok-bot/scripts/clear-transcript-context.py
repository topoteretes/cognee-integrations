#!/usr/bin/env python3
"""Clear the Grok transcript context after each assistant response.

Hooks cannot execute the built-in /clear command directly. This
Stop hook is the integration-level demo workaround: when enabled, it empties
the transcript file the host passes in the hook payload.
"""
...
ENV_NAME = "COGNEE_GROK_CLEAR_AFTER_MESSAGE"
TRUTHY = {"1", "true", "yes", "on"}
PLUGIN_DIR = Path.home() / ".cognee-plugin" / "grok-bot"
...
def main() -> int:
    ...
    try:
        ...
        if isinstance(payload, dict):  # Grok's native hook payload is camelCase
            for camel, snake in (
    ...
    output = {
        "systemMessage": "Cognee demo: Grok transcript context was emptied.",
        "suppressOutput": True,
    }
