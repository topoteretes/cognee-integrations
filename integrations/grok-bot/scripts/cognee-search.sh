...
#   grok-bot/sessions/<host id>.json — follows a dataset switch), falling
#   back to the Cognee connection endpoint and COGNEE_PLUGIN_DATASET / agent_sessions.

set -euo pipefail

PLUGIN_DIR="${HOME}/.cognee-plugin/grok-bot"
...
[ -z "$SESSION_ID" ] && SESSION_ID="${COGNEE_SESSION_ID:-grok_session}"
