"""Shared configuration for the Cognee Grok Bot plugin.
...
COGNEE_GROK_BACKEND does the same for this plugin only, beating the shared
...
_DEFAULTS = {
    ...
    "agent_name": "grok-bot-agent",
    "session_strategy": "per-directory",  # per-directory | git-branch | static
    "session_prefix": "grok",  # agent name; session id is "{agent}_{host_session_id}"
    "top_k": 3,
    "backend": "auto",
    ...
    "llm_api_key": "",
    "llm_model": "",
    # Memory steering: assert Cognee as the preferred memory over Grok's
...
def get_session_id(config: dict, cwd: Optional[str] = None) -> str:
    ...
    saves/recalls. The host (Grok) session id is read from the in-process
    ...
    if cwd is None:
        cwd = os.environ.get("GROK_CWD", os.getcwd())
