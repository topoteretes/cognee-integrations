...
If the session lists a `cognee-memory` (or `cognee`) MCP server, call its
`recall` tool once with the query (and `top_k` 10) and answer from that — it
queries the same server and dataset as the wrapper below. Otherwise:
...
```bash
${GROK_PLUGIN_ROOT}/scripts/cognee-search.sh "<query>" 10
```
...
- `/cognee-sync` to sync session data to the permanent graph
- `/cognee-remember` to ingest new data
