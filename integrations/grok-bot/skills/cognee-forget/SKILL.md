...
- `forget(dataset_id=..., data_id=...)` deletes one document; call it only
  after the user confirmed exactly what to delete, and never with a
  dataset-wide or `everything` scope unless they explicitly asked for that.
...
```bash
${GROK_PLUGIN_ROOT}/scripts/cognee-forget.sh sync
```
...
```bash
${GROK_PLUGIN_ROOT}/scripts/cognee-forget.sh datasets
```
...
```bash
${GROK_PLUGIN_ROOT}/scripts/cognee-forget.sh data <dataset_id>
```
...
```bash
${GROK_PLUGIN_ROOT}/scripts/cognee-forget.sh raw <dataset_id> <data_id>
```
...
```bash
${GROK_PLUGIN_ROOT}/scripts/cognee-forget.sh forget <dataset_id> <data_id>
```
...
```bash
eval "$(${GROK_PLUGIN_ROOT}/scripts/cognee-forget.sh env)" && \
