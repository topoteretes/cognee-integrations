...
```bash
python3 ${GROK_PLUGIN_ROOT}/scripts/sync-session-to-graph.py
```
...
After this, session entries become searchable via `/cognee-search`, and the graph knowledge is automatically included in session completion prompts.
