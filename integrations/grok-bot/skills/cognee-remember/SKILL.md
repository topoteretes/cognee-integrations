...
- `remember(data=<text>, node_set=["user_context" | "project_docs" | "agent_actions"])`
  stores permanently in the graph; pass `session_id` to write to the session
  cache instead.
...
```bash
${GROK_PLUGIN_ROOT}/scripts/cognee-remember.sh "$ARGUMENTS" --node-set user_context
```

**Project data** (docs, code, company knowledge):
```bash
${GROK_PLUGIN_ROOT}/scripts/cognee-remember.sh "$ARGUMENTS" --node-set project_docs
```

**Agent data** (explicit agent notes — routine tool logs are automatic):
```bash
${GROK_PLUGIN_ROOT}/scripts/cognee-remember.sh "$ARGUMENTS" --node-set agent_actions
```
...
```bash
${GROK_PLUGIN_ROOT}/scripts/cognee-remember.sh --file src/payments.py --node-set project_docs
```
