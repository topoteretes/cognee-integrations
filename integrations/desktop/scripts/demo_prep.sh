...
start_backend() { # $1 profile, $2 port
  if ! curl -s -m 2 "localhost:$2/health" > /dev/null 2>&1; then
    COGNEE_DESKTOP_PROFILE="$1" COGNEE_DESKTOP_SEARCH_CACHE_TTL=7200 nohup ./scripts/run_backend.sh \
      > "/tmp/cognee-desktop-backend-$1.log" 2>&1 &
...
set -a; source "$HOME/.cognee-desktop/backend.env" 2>/dev/null; set +a
...
curl -s -m 180 'localhost:8766/search?q=why%20is%20cognee%20search%20fast%20now&mode=answer' > /dev/null 2>&1 || true
curl -s -m 30 'localhost:8766/search?q=cognee' > /dev/null 2>&1 || true
...
# the app itself
if ! pgrep -x CogneeDesktop > /dev/null; then
  open "$ROOT/macos/dist/Cognee.app"
  sleep 3
fi
pgrep -x CogneeDesktop > /dev/null && ok "app running (⌥Space)" || bad "app not running"
