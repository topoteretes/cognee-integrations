...
launcher-style panel, shareable person-to-person, and remembered by your
coding agents.
...
```bash
cd integrations/desktop && ./scripts/demo_prep.sh
```
...
## Beat 1 — "System search, but it understands meaning" (2 min)
...
- The **memory layer** switcher (top right) flips between datasets: your files
...
the desktop integration?"* — the plugin recalls the same tenant memory.
...
4. The kicker: ⌥ Space (still as boris) → **`why is cognee search fast
...
team's lessons. This panel is just the window into it."

## Optional flex — memory that argues back (1 min)

Needs `COGNEE_DESKTOP_EXPERIMENTS=1` in `~/.cognee-desktop/backend.env`
...
| Total meltdown | `COGNEE_DESKTOP_DEBUG_QUERY=meridian "macos/dist/Cognee.app/Contents/MacOS/CogneeDesktop"` auto-opens the panel with results for a screenshot-grade fallback |
