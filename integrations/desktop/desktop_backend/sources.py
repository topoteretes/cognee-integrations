...
class MockConnectorSource:
    ...
    Enabled with ``COGNEE_DESKTOP_MOCK_SOURCES=slack,gdrive``. Content is written
...
class SourceManager:
    ...
    def from_env(cls, indexer: Any, data_dir: Path) -> "SourceManager":
        manager = cls(indexer, data_dir, interval=float(env("SYNC_INTERVAL", "60")))
        if env("WATCH_FOLDERS", "true").lower() != "false":
