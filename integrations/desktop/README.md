...
System-wide memory search for macOS, powered by [cognee](https://github.com/topoteretes/cognee).
...
| Piece | What it is |
|---|---|
| `macos/` | Native SwiftUI/AppKit menu-bar app: global hotkey, floating non-activating search panel, settings window. Builds into a real `Cognee.app` with Command Line Tools only — no Xcode. |
| `desktop_backend/` | Small FastAPI server the app talks to. Walks your folders, feeds files into cognee, and merges instant filename matches with cognee's semantic results. |
...
- **esc** (or clicking elsewhere) dismisses the panel, launcher-style
...
`~/.cognee-desktop/backend.env` (chmod 600, sourced by `run_backend.sh`
...
Same app, same panel — files are uploaded to the server's own dataset
...
Configure identity + hub in `.env` (`COGNEE_DESKTOP_USER`, `COGNEE_DESKTOP_TEAM`,
...
# 2. two identities
COGNEE_MODE=fake COGNEE_DESKTOP_PORT=8767 COGNEE_DESKTOP_USER=vasilije COGNEE_DESKTOP_TEAM=core \
  COGNEE_DESKTOP_DATA_DIR=/tmp/demo/senior COGNEE_HUB_URL=http://127.0.0.1:8011 \
  uv run python -m desktop_backend
COGNEE_MODE=fake COGNEE_DESKTOP_PORT=8766 COGNEE_DESKTOP_USER=boris COGNEE_DESKTOP_TEAM=core \
  COGNEE_DESKTOP_DATA_DIR=/tmp/demo/junior COGNEE_HUB_URL=http://127.0.0.1:8011 \
  uv run python -m desktop_backend
...
posture. The transport is one small class (`desktop_backend/handover.py`)
...
directories, and files over 5 MB. State lives in `~/.cognee-desktop/`.
...
│   └── scripts/make_app.sh             # SPM build → Cognee.app
└── scripts/run_backend.sh
```
