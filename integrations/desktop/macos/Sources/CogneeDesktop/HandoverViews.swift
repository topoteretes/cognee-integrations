    ...
    func refresh() {
        Task {
            do {
                ...
                statusText = enabled ? "" : "Handover not configured — set COGNEE_DESKTOP_USER and COGNEE_HUB_URL for the backend."
            } catch {
...
struct ShareView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text("Send distilled knowledge to a teammate's memory. They get a notification, and it becomes searchable in their Cognee panel.")
                .font(.callout)
                .foregroundStyle(.secondary)
            Form {
