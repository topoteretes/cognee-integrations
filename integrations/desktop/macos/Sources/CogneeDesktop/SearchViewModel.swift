...
final class SearchViewModel: ObservableObject {
    ...
    /// Breadcrumbs to /tmp when COGNEE_DESKTOP_DEBUG_QUERY is set — the panel has
    /// no console, so silent failures need a paper trail.
    static let debugging = ProcessInfo.processInfo.environment["COGNEE_DESKTOP_DEBUG_QUERY"] != nil

    static func debugLog(_ message: String) {
        guard debugging else { return }
        let line = "\(Date()) \(message)\n"
        if let data = line.data(using: .utf8),
            let handle = FileHandle(forWritingAtPath: "/tmp/cognee-desktop-app.log") {
            handle.seekToEndOfFile()
            handle.write(data)
            try? handle.close()
        } else {
            try? line.write(
                toFile: "/tmp/cognee-desktop-app.log", atomically: false, encoding: .utf8)
        }
    }
    ...
    func originLabel(for path: String) -> String? {
        ...
        if path.contains("/.cognee-desktop/"), path.contains("/capture/") {
            return "Captured"
        }
        return nil
    }
    ...
    private func sharePayload() -> (title: String, body: String, source: String)? {
        if let answer = answer {
            return (query, answer, "cognee answer")
        }
