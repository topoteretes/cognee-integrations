...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        let autosaveName = "CogneeDesktop"
        ...
        // Headless debugging: COGNEE_DESKTOP_DEBUG_QUERY=<q> auto-opens the panel and
        // runs a search, so panel rendering can be verified via screenshot.
        if let debugQuery = ProcessInfo.processInfo.environment["COGNEE_DESKTOP_DEBUG_QUERY"] {
        ...
        let forceOnboarding =
            ProcessInfo.processInfo.environment["COGNEE_DESKTOP_DEBUG_ONBOARDING"] != nil
    ...
    private static func menuBarIcon(unread: Bool = false) -> NSImage? {
        guard let url = Bundle.main.url(forResource: "MenuIcon", withExtension: "png"),
            ...
            return NSImage(
                ...
                accessibilityDescription: "Cognee"
            ) ?? NSImage(
                systemSymbolName: "magnifyingglass", accessibilityDescription: "Cognee")
        }
    ...
    private func buildMenu() -> NSMenu {
        ...
        menu.addItem(.separator())
        menu.addItem(
            NSMenuItem(title: "Quit Cognee", action: #selector(NSApp.terminate(_:)),
                       keyEquivalent: "q")
        )
        return menu
    }
