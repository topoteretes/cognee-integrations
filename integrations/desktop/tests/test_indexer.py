...
from desktop_backend.indexer import discover_files
...
async def test_dataset_overrides_route_files_and_cognify(tmp_path):
    ...
    from desktop_backend.catalog import Catalog
    from desktop_backend.indexer import Indexer
    ...
    adapter = FakeAdapter(dataset="main")
    ...
    assert adapter.dataset_of[str(docs / "note.md")] == "main"
    assert adapter.dataset_of[str(repo / "issue-1.md")] == "github-acme-rockets"
    # both datasets got their graphs built
    assert set(adapter.cognified_datasets) == {"main", "github-acme-rockets"}
